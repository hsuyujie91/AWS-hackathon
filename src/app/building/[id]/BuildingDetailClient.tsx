...
import { buildingLevelFromMinutes, minutesToNextLevel, nextLevelThreshold, XP_RULES } from "@/lib/xp";
...
export function BuildingDetailClient() {
  ...
  function watch(course: Course) {
    ...
    const leveledUp = buildingLevelFromMinutes(building!.minutes + minutes) > building!.level;
    dispatch({ type: "WATCH_COURSE", courseId: course.id, minutes });
    setSuccess({ xp: minutes * XP_RULES.watchPerMinute, buildingName: building!.name, leveledUp });
  }
