...
export default function TasksPage() {
  ...
  function complete(task: DailyTask) {
    if (task.done || activeTaskId !== task.id) return;
    const building = state.buildings.find((item) => item.id === task.buildingId)!;
    const leveledUp = buildingLevelFromMinutes(building.minutes + task.estMinutes) > building.level;
    dispatch({ type: "COMPLETE_TASK", taskId: task.id });
    setActiveTaskId(null);
    setSuccess({ xp: task.xp, coins: task.coins, buildingName: building.name, leveledUp, message: "獎勵已立即加入帳戶，不需要等其他任務完成。" });
  }
      ...
      <div className="mx-auto grid max-w-[1380px] items-start gap-4 xl:grid-cols-[430px_minmax(0,1fr)]">
        <aside className="space-y-4 xl:sticky xl:top-5">
          <section className="relative min-h-[315px] overflow-hidden rounded-[26px] border border-indigo-100 bg-gradient-to-br from-white via-indigo-50 to-blue-100 p-6 shadow-sm">
            ...
            <div className="absolute bottom-2 right-2 h-44 w-44"><Image src="/assets/icons/ai-robot.png" alt="AI 助教" fill priority sizes="176px" className="object-contain drop-shadow-[0_18px_16px_rgba(79,70,229,.2)]" /></div>
          </section>
          ...
          <section className="rounded-[24px] border border-white/80 bg-white/95 p-5 shadow-sm">
            ...
            <div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-amber-50 p-4"><Image src="/assets/badges/level-badge.png" alt="經驗值" width={34} height={34} className="h-9 w-9 object-contain" /><p className="mt-2 text-[11px] font-bold text-slate-500">已獲得經驗值</p><p className="text-lg font-black text-amber-700">{earnedXp} / {totalXp} XP</p></div><div className="rounded-2xl bg-yellow-50 p-4"><Image src="/assets/badges/star-coin.png" alt="金幣" width={34} height={34} className="h-9 w-9 object-contain" /><p className="mt-2 text-[11px] font-bold text-slate-500">已獲得金幣</p><p className="text-lg font-black text-yellow-700">{earnedCoins} / {totalCoins}</p></div></div>
          </section>
        ...
        <main className="rounded-[26px] border border-white/80 bg-white/95 p-4 shadow-sm sm:p-6">
          ...
          <div className="mt-5 space-y-3">
            {state.tasks.map((task, index) => {
              ...
              return <article key={task.id} className={`relative overflow-hidden rounded-[22px] border p-4 transition sm:p-5 ${task.done ? "border-emerald-100 bg-emerald-50/50" : active ? "border-indigo-400 bg-indigo-50/50 shadow-[0_14px_35px_-24px_rgba(79,70,229,.75)]" : "border-slate-200 bg-white hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md"}`}>
                <div className="grid items-center gap-4 sm:grid-cols-[64px_minmax(0,1fr)_180px]">
                  ...
                  <div><div className="mb-3 flex items-center justify-center gap-3 rounded-xl bg-slate-50 p-2.5"><span className="flex items-center gap-1 text-xs font-black text-amber-600"><Image src="/assets/badges/level-badge.png" alt="" width={20} height={20} className="h-5 w-5 object-contain" />+{task.xp} XP</span><span className="h-5 w-px bg-slate-200" /><span className="flex items-center gap-1 text-xs font-black text-yellow-700"><Image src="/assets/badges/star-coin.png" alt="" width={18} height={18} className="h-4 w-4 object-contain" />+{task.coins}</span></div>{task.done ? <div className="flex items-center justify-center gap-2 rounded-xl bg-emerald-100 py-3 text-xs font-black text-emerald-700"><CheckCircle2 className="h-4 w-4" />獎勵已領取</div> : active ? <button onClick={() => complete(task)} className="w-full rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 py-3 text-xs font-black text-white shadow-lg shadow-emerald-100">完成並領取獎勵</button> : <button onClick={() => setActiveTaskId(task.id)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 py-3 text-xs font-black text-white shadow-lg shadow-indigo-100"><Play className="h-4 w-4" />開始任務</button>}</div>
                </div>
              </article>;
            })}
          </div>
